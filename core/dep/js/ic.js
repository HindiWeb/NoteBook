$(document).ready(function() {
    ...
    $('body').on('click','.note.btn',function(){
        ...
        $notewall  =  topic.find('.notewall');
        $notewall.find('.editBtn').hide();
        $editor = $notewall.find('.editor')
        $editor.removeClass('ql-container ql-snow')
        $editor.html(`
            <div class="text-center">
                <div class="text-center m-auto loader sm"></div>
            </div>`);
        $notewall.find('.ql-toolbar').remove();
